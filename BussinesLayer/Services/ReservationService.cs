    ...
    : IReservationServices
{
    public async Task<GResponse<ReservationsDto>> AddReservation(ReservationsAddDto reservation)
    {
        ...
        await using (@lock)
        {
            ...
            dto.OrgName = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.Org!.OrgName).FirstOrDefault()!;
            dto.OrgSerial = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.QueueSerial).FirstOrDefault()!;
            dto.QueueSerial = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.QueueSerial).FirstOrDefault()!;
          ...
          //  await _hubContext.Clients.All.SendAsync("AddReservation", dto);
            ...
            var message = $"مرحبا بكم بالمستشفى البريطاني تم حجز طلبكم بنجاح والرقم الخاص بكم هو  {reservationEntity.TicketNumber}";
    ...
    public async Task<GResponse<ReservationsDto>> CallNextInQueue(CallNextInqueueReq callnextinqueuereq)
    {
        ...
        using (@lock)
        {
            ...
            if (callnextinqueuereq.ServiceId != 0)
            {
                 counterName = await _counterServicesRepository.Where(x => x.CounterId == callnextinqueuereq.CounterId && x.ServiceId == callnextinqueuereq.ServiceId)!
                    .Include(x => x.Counter)
            ...
            {
                 counterName = await _counterServicesRepository.Where(x => x.CounterId == callnextinqueuereq.CounterId)!
                    .Include(x => x.Counter)
                    .Select(x => x.Counter!.CounterName)
                    .FirstOrDefaultAsync();
            }
            ...
            await _repository.Commit();
            var dto = _mapper.Map<ReservationsDto>(reservation);
            dto.QueueSerial = reservation.QueueSerial;
            dto.CounterId = callnextinqueuereq.CounterId;
            dto.OrgName = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.Org!.OrgName).FirstOrDefault()!;
            dto.OrgSerial = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.QueueSerial).FirstOrDefault()!;
            dto.QueueSerial = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.QueueSerial).FirstOrDefault()!;
            dto.CreatedOn = DateTime.Now;
            dto.ReqId = reservation.ServiceId;
            dto.Services = _repository.Where(x => x.Id == dto.Id)!.Select(x => x.ReservationsServices!.Select(x => x.Service!.ServiceName).ToList()).FirstOrDefault()!;
            ...
            var password = _appConfig.SmsSetteings!.Password;
            var sender = _appConfig.SmsSetteings!.api_key;
            var message = $"عميلنا العزيز برجاء التوجه الي شباك رقم  {counterName}  لتلقي الخدمه الخاصه بكم";
            var client = new HttpClient();
            var request = new HttpRequestMessage(HttpMethod.Get, $"https://api.epusheg.com/api/v2/send_bulk?username={username}&password={password}&api_key={sender}&message={message}&from=BritishHosp&to={reservation.MobileNumber}");
            var response = await client.SendAsync(request);
            try
    ...
    public async Task<GResponse<List<ReservationsDto>>> GetReservationsBasedOnOrgId(int orgId)
    {
        var reservations = await _repository
            ...
            .Include(x => x.Org)
            .Include(r => r.ReservationsServices)
                .ThenInclude(rs => rs.Service)  // Ensure services are included
            .ToListAsync();

        if (reservations == null || !reservations.Any())
        {
            return GResponse<List<ReservationsDto>>.CreateFailure("404", "No reservations found.");
        }

        var dto = reservations.Select(r => new ReservationsDto
        {
            Id = r.Id,
            OrgName = r.Org?.OrgName ?? string.Empty,
            QueueSerial = r.QueueSerial,
            OrgSerial = r.QueueSerial,
            CreatedOn = DateTime.Now,
            ReqId = r.ServiceId ?? 0,
            Status = r.Status,
            Orgid = r.Orgid,
            ReservationType = r.ReservationType,
            BookingDetails = r.Service?.ServiceName,
            CallAt = r.CallAt,
            CitizenId = r.CitizenId ?? 0,
            CounterId = r.CounterId ?? 0,
            CustomerId = r.CustomerId ?? 0,
            EndServing = r.EndServing,
            MobileNumber = r.MobileNumber,
            Name = r.Name,
            Nid = r.Nid,
            ReservationDate = r.ReservationDate,
            TicketNumber = r.TicketNumber,
            Services = r.ReservationsServices?.Select(rs => rs.Service?.ServiceName).Where(s => s != null).ToList() ?? new List<string>()
        }).ToList();
